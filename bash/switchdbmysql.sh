echo "停止zfile..."
bash ./zfile/bin/stop.sh  #停止zfile
...
sed -i '21c spring.datasource.driver-class-name='"$dbclassname" ./zfile/WEB-INF/classes/application.properties  #原来sed可以这么添加内容？反正我百度sed使用方法的时候没看到，自己整出来的


echo -n "请输入mysql数据库地址："
read dburl
sed -i '22c spring.datasource.url='"$dburl" ./zfile/WEB-INF/classes/application.properties


echo -n "请输入mysql数据库用户名："
read dbusername
sed -i '23c spring.datasource.driver-class-name='"$dbusername" ./zfile/WEB-INF/classes/application.properties


echo -n "请输入mysql数据库密码："
read dbpassword
sed -i '24c spring.datasource.dbpassword='"$dbpassword" ./zfile/WEB-INF/classes/application.properties


echo "修改完成，请重新启动zfile"
